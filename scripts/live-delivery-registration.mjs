...
const dir=await mkdtemp(resolve('.tmp/live-delivery-register-')),workspace=`${dir}-workspace`;await mkdir(workspace);
...
const moduleFile=join(dir,'store.mjs');await build({entryPoints:['src/state/workbench.ts'],outfile:moduleFile,bundle:true,platform:'node',format:'esm'});const {WorkbenchStore}=await import(pathToFileURL(moduleFile).href);const store=new WorkbenchStore(join(dir,'workbench.sqlite'));const bot=store.create(workspace,{name:'文章编辑',role:'按用户要求创建文件，实际保存并交付，不把不能预览误认为不能交付。',kind:'bot',members:[]});store.select(workspace,bot);store.close();
...
try{
 app=await electron.launch({executablePath:resolve('build/feedback-delivery-paste/mac-arm64/FormaBot.app/Contents/MacOS/FormaBot'),env,timeout:30000});const page=await app.firstWindow();await page.locator('#workbench').waitFor();await page.waitForFunction(()=>!document.querySelector('#run').disabled);await app.evaluate(({dialog})=>{dialog.showMessageBox=async()=>({response:0,checkboxChecked:false});});
 for(const [label,prompt] of [['ordinary','请创建两份测试文件并交付：outputs/check.pl 内容为 print \"ok\\n\";，outputs/check.md 内容为“测试说明”。不用运行脚本。结束时把这两个文件都列入 task_result 的 artifacts，确认它们实际保存成功。']]){
  ...
  if(label==='ordinary'){
   assert.ok(message.artifacts.length);const article=await readFile(message.artifacts[0],'utf8');assert.ok(article.length>0);assert.ok(message.artifacts.some(p=>p.endsWith('.pl')));assert.ok(message.artifacts.some(p=>p.endsWith('.md')));
